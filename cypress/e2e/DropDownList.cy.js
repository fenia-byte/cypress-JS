...
describe("dropdownList", () => {
  it("Verify link and default value", () => {
    ...
    cy.get(basePage.LOCATORS.content).should(
      "contain",
      dropdownList.NAMES.contentText
    );

    cy.get(basePage.getExample()).get("select").as("selectDropdown");
    cy.get("@selectDropdown")
      .find("option:selected")
      .should("have.text", dropdownList.NAMES.options.option0);
  });
  ...
  it("Verify dropdown's options count is 3", () => {
    ...
    cy.get(basePage.getExample())
      .get(dropdownList.NAMES.dropdownID)
      .find("option")
    ...
    for (let i = 1; i <= 2; i++) {
      for (let j = 1; j <= 2; j++) {
        if (j !== i) {
          ...
          cy.get("@selectDropdown")
            .find("option:selected")
            .should("have.text", dropdownList.NAMES.options[`option${i}`]); //Positive case
          cy.get("@selectDropdown")
            .find("option:selected")
            .should("not.have.text", dropdownList.NAMES.options[`option${j}`]);

          cy.get("@selectDropdown")
            .get("select option:first")
            .should("be.disabled"); //check the 1-st (default) option is disabled
        }
      }
    }
  });
